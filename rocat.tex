...
Furthermore, Rocat can visualize the relationship of source code with the program structure at the same time.
For example, in the case of the call graph visualization, 2D visualization can represent the relationship by using nodes and edges, but it is difficult to visualize other metrics clearly on one window.
On the other hand, Rocat can visualize the relationship like the bridge between buildings.
The developers can understand the call graph and the program structure at the same time.
This approach will be able to accelerate the debug task.
...
We would like to thank Yusuke Saito and Shin Fujiwara for their initial work on Rocat, and Takao Nakagawa and Stevche Radevski for their suggestions and advices.
