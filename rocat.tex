...
\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{ogami.pdf}
\caption{RocatDebugger visualization for call graph}
...
Furthermore, we are developing "RocatDebugger" that visualizes Java programs' call graph and stack traces in real time.
\figref{figure:Debugger} shows a screen capture of RocatDebugger.
RocatDebugger is implemented as an Eclipse plugin, and assists developers to understand programs' behavior in debug.
We would like to evaluate it with questionnaire and task of program comprehension for developers.
